---
layout: cv
permalink: /cv/
title: cv
nav: true
nav_order: 3
description: Curriculum Vitae - Zengyi Wo
toc:
  sidebar: left
---

## Experience

**Baidu Inc.**, Beijing, China
- Algorithm Engineer, Multimodal LLM Post-Training
- Jul 2025 – Present
- Working on Supervised Fine-Tuning (SFT), Reinforcement Learning (RL), and Online Preference Distillation (OPD) for multimodal large language models

**Duxiaoman Finance** (Apr 2025 – Aug 2025)
- Project Lead, LLM Post-Training and Domain Alignment
- Developed SFT/RLHF pipelines for financial domain LLMs

## Education

**Tianjin University**, Tianjin, China
- M.S. in Computer Science and Technology, College of Intelligence and Computing
- Sep 2022 – Jun 2025
- Advisor: Prof. Wenjun Wang, Prof. Minglai Shao
- Collaborators: Prof. Lu Lin (Penn State), Prof. Ruijie Wang (UIUC)
- Research Focus: Graph Machine Learning, Large Language Models

**Zhengzhou University**, Zhengzhou, China
- B.S. in Computer Science and Technology
- Sep 2018 – Jun 2022

## Research Interests

- **Graph Machine Learning**: Graph Neural Networks, Graph Contrastive Learning, Graph Anomaly Detection
- **Large Language Models**: Post-Training (SFT/RLHF), Domain Adaptation, Parameter-Efficient Fine-Tuning
- **Multimodal AI**: Graph-Language Models, Cross-modal Alignment

## Publications

See [Publications](/publications/) page for full list.

**Selected Publications:**
- **AAAI 2026** (Oral): LLMTM: Benchmarking and Optimizing LLMs for Temporal Motif Analysis
- **KDD 2025** (Oral): Local Homophily-Aware Graph Neural Network for Scalable Graph Anomaly Detection
- **WWW 2024** (Oral): Graph Contrastive Learning via Interventional View Generation
- **IJCAI 2025**: Leveraging Personalized PageRank for Heterophily Mitigation in GNNs
- **DASFAA 2025**: Unsupervised Multiplex Graph Representation Learning

## Honors & Awards

- KDD 2025 Oral Presentation (Acceptance Rate ~18.4%)
- WWW 2024 Oral Presentation (Acceptance Rate ~20.2%)
- National Scholarship for Graduate Students, 2023
- Outstanding Graduate Student Award, Tianjin University, 2024

## Academic Services

- **Reviewer**: NeurIPS, ICML, ICLR, KDD, AAAI, IJCAI
- **Program Committee**: DASFAA 2025

## Skills

- **Programming**: Python, PyTorch, C++
- **Frameworks**: PyTorch Geometric, DeepSpeed, Transformers, LangChain
- **Tools**: Git, Docker, Linux, Neo4j
- **Languages**: Chinese (Native), English (Fluent)

## Contact

- Email: wozengyi1999@tju.edu.cn
- GitHub: [github.com/wozengyi](https://github.com/wozengyi)
- Google Scholar: [scholar.google.com/citations?user=n7x0S2MAAAAJ](https://scholar.google.com/citations?user=n7x0S2MAAAAJ)
